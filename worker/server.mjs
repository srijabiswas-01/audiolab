...
async function audioFromYouTube(url) {
  ...
  try {
    await run(['--no-playlist', '--no-progress', '--extract-audio', '--audio-format', 'wav', '--max-filesize', '100M', '--match-filter', 'duration <= 600', '--output', path.join(directory, '%(id)s.%(ext)s'), url]);
